			...
			while (!reader.EndOfStream)
			{
				...
				if (flag && lastReadChar == '\\')
				{
					...
					case 'u':
					{
						...
						Context.Config.DebugContext.LogError("A wild non-hex value appears at position " + reader.BaseStream.Position + "! \\-u-" + c2.ToString() + "-" + c3.ToString() + "-" + c4.ToString() + "-" + c5.ToString() + "; current buffer: '" + new string(buffer, 0, bufferIndex + 1) + "'. If the error handling policy is resilient, an attempt will be made to recover from this emergency without a fatal parse error...");
						lastReadChar = null;
						if (emergencyPlayback == null)
						{
				...
				else if (EntryDelineators.TryGetValue(c, out value))
				{
					if (value.HasValue)
					{
						...
						case EntryType.StartOfNode:
						{
							ConsumeChar();
							EntryType entry3;
							ParseEntryFromBuffer(out name, out valueContent, out entry3, num, EntryType.StartOfNode);
							break;
						}
						case EntryType.PrimitiveArray:
						{
							ConsumeChar();
							EntryType entry2;
							ParseEntryFromBuffer(out name, out valueContent, out entry2, num, EntryType.PrimitiveArray);
							break;
						}
