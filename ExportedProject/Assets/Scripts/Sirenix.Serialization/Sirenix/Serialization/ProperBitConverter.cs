		...
		public static byte[] HexStringToBytes(string hex)
		{
			...
			for (int i = 0; i < num; i++)
			{
				...
				try
				{
					...
					if (b == byte.MaxValue)
					{
						throw new ArgumentException("Expected a hex character, got '" + hex[num2].ToString() + "' at string index '" + num2 + "'.");
					}
				}
				...
				try
				{
					...
					if (b2 == byte.MaxValue)
					{
						throw new ArgumentException("Expected a hex character, got '" + hex[num2 + 1].ToString() + "' at string index '" + (num2 + 1) + "'.");
					}
				}
				catch (IndexOutOfRangeException)
				{
