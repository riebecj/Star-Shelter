	...
	public class SerializationNodeDataWriter : BaseDataWriter
	{
		...
		public override void BeginReferenceNode(string name, Type type, int id)
		{
			Nodes.Add(new SerializationNode
			{
				...
				Data = ((type != null) ? (id.ToString(CultureInfo.InvariantCulture) + "|" + base.Binder.BindToName(type, base.Context.Config.DebugContext)) : id.ToString(CultureInfo.InvariantCulture))
			});
		...
		public override void BeginStructNode(string name, Type type)
		{
			Nodes.Add(new SerializationNode
			{
				Name = name,
				Entry = EntryType.StartOfNode,
				Data = ((type != null) ? base.Binder.BindToName(type, base.Context.Config.DebugContext) : "")
			});
